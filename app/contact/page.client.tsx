...
export function ContactPageClient({
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const selectedServices = data.getAll('service_interest').join(', ') || 'None selected';
    ...
    const body = [
      ...
      `Company: ${company}`,
      `Phone: ${phone}`,
      `Service Interest: ${selectedServices}`,
  ...
  const inputClass =
    'w-full px-4 py-3 rounded-xl text-sm text-white placeholder:text-white/20 focus:outline-none transition-all appearance-none';
  const inputStyle = {
    ...
    <>
      ...
      <section ref={heroRef} className="relative z-10 pt-40 pb-16 px-6 lg:px-8 max-w-6xl mx-auto">
        ...
        <p className="text-base leading-relaxed max-w-xl"
          ...
          {subheading || "No pitch decks or sales pitch, we\u2019re just here to listen and find out what is holding your business back from growing. If that sounds reasonable, let\u2019s talk."}
        </p>
      </section>
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold mb-1.5" style={{ color: 'rgba(240,244,255,0.45)' }}>
                ...
                <ServicePicker label="Services You Need" services={services} />
                <div>
                  <label className="block text-xs font-semibold mb-1.5" style={{ color: 'rgba(240,244,255,0.45)' }}>
                  ...
                  <div className="relative">
                    ...
                    <svg className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4"
                      style={{ color: 'rgba(240,244,255,0.3)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>

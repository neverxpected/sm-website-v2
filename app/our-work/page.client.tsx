...
import type { CaseStudy, Json } from '@/types/database';
...
const ACCENT_PALETTE = [
  { accentColor: 'rgba(255,45,120,0.15)', accentBorder: 'rgba(255,45,120,0.3)', glowColor: 'rgba(255,45,120,0.12)' },
  { accentColor: 'rgba(155,48,255,0.15)', accentBorder: 'rgba(155,48,255,0.3)', glowColor: 'rgba(155,48,255,0.12)' },
  { accentColor: 'rgba(0,229,255,0.12)', accentBorder: 'rgba(0,229,255,0.25)', glowColor: 'rgba(0,229,255,0.10)' },
];

function parseCaseStudy(cs: CaseStudy, index: number): CaseStudyDisplay {
  const palette = ACCENT_PALETTE[index % ACCENT_PALETTE.length];
  const stats = cs.stats as Json;

  let primaryStat: StatDisplay = { value: 0, prefix: '', suffix: '', label: '' };
  const meta: string[] = [];

  if (Array.isArray(stats) && stats.length > 0) {
    const first = stats[0] as Record<string, Json | undefined>;
    const rawValue = String(first.value ?? '0');
    const numericStr = rawValue.replace(/[^0-9.]/g, '');
    const numericVal = Math.round(parseFloat(numericStr) || 0);

    const prefixMatch = rawValue.match(/^([^0-9]*)/);
    const suffixMatch = rawValue.match(/([^0-9.]*)$/);
    ...
    for (let i = 1; i < stats.length; i++) {
      const s = stats[i] as Record<string, Json | undefined>;
      const emoji = s.emoji ? `${s.emoji} ` : '';
      meta.push(`${emoji}${s.label ?? ''}`);
    }
  }

  if (cs.category && !meta.includes(cs.category)) {
    meta.push(cs.category);
  }
...
function StatCard({ study, index }: { study: CaseStudyDisplay; index: number }) {
  const ref = useRef<HTMLDivElement>(null);
  const [visible, setVisible] = useState(false);
  const [hovered, setHovered] = useState(false);
  const count = useCountUp(study.stat.value, 1800, visible);
    ...
    >
      ...
      <div className="flex flex-col gap-1">
        <span className="text-5xl font-black tracking-tight" style={{
          ...
          {study.stat.prefix}{count.toLocaleString()}{study.stat.suffix}
        </span>
        <span className="text-sm font-bold uppercase tracking-[0.15em]" style={{ color: 'rgba(240,244,255,0.45)' }}>
          {study.stat.label}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {study.meta.map((tag) => (
          <span key={tag} className="px-3 py-1 rounded-full text-xs font-bold" style={{
...
export function OurWorkPageClient({
  ...
  const displayStudies = caseStudies.map((cs, i) => parseCaseStudy(cs, i));
  ...
  return (
    <>
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute top-1/4 left-1/4 w-[800px] h-[800px] rounded-full" style={{
          background: 'radial-gradient(circle, rgba(255,45,120,0.22) 0%, rgba(255,45,120,0.08) 40%, transparent 70%)',
          animation: 'floatOrb 12s ease-in-out infinite',
        }} />
        <div className="absolute bottom-1/4 right-1/4 w-[700px] h-[700px] rounded-full" style={{
          background: 'radial-gradient(circle, rgba(155,48,255,0.22) 0%, rgba(155,48,255,0.08) 40%, transparent 70%)',
          animation: 'floatOrb 16s ease-in-out infinite reverse',
        }} />
        <div className="absolute top-2/3 left-1/2 w-[500px] h-[500px] rounded-full" style={{
      ...
      <section className="relative z-10 pb-24 px-6 lg:px-8 max-w-5xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {displayStudies.map((study, i) => (
            <StatCard key={study.id} study={study} index={i} />
          ))}
        </div>
      </section>
      ...
      <section className="relative z-10 pb-32 px-6 lg:px-8 max-w-5xl mx-auto text-center">
        <RevealSection delay={0}>
          <div className="p-12 rounded-3xl" style={{
